    ...
    class ResourceManager {
        ...
        public function add(Resource $resource){ //add new resource
            ...
            if(!$result){ //doesn't exist yet then create
                ...
                try{ //update
                    $sth->execute();
                    return [true, 'Ressource créée', 201];
                }catch(PDOException $e){
                    return [false, "Erreur dans l'execution de la requête", 400];
                }
            }else{ // exist in this technology and wasn't delete 
                return [false, 'Ressource déjà existante avec cette technologie', 403];
            }
        }
        ...
        public function getList(){ //display all resources 
            ...
            try{ 
                ...
                if($displayResult){ 
                    ...
                    return [false, "Resources inexistantes", 404];
                }
        ...
        public function get($id){ //display one resource
            ...
            if(!$result){ //doesn't exist
                return [false, "Resource inexistante", 404];
        ...
        public function getListFor($idTechnology){ //display all resources 
            $displayResult = false; //bool for check if there is something to display
            $sql = "SELECT r.id, r.url 
                    FROM (resource AS r 
                    INNER JOIN technology AS t ON t.id = r.technology_id) 
                    WHERE r.deleted = 0 AND r.technology_id = :idTechnology";
            $sth = $this->connection->prepare($sql);
            $sth->bindParam(':idTechnology', $idTechnology, PDO::PARAM_INT);
            try{ 
                $sth->execute();
                while ($data = $sth->fetch(PDO::FETCH_ASSOC)){
                    $dataResource = ["id" => $data["id"], "url" => $data["url"]];
                    $result[] = [new Resource($dataResource)]; 
                    $displayResult = true;
                }; 
                ...
                if($displayResult){ 
                    ...
                    return [false, "Resources inexistantes pour cette technologie", 404];
                }
            }catch(PDOException $e){ //some error in sql execution
                return [false, "Erreur dans l'execution de la requête", 400];
            }
            ...
            if(!$result){ //doesn't exist 
                ...
                if ($result[0]["deleted"] == 0){ //exist and wasn't deleted before
                    ...
                    if($update){
                        ...
                        try{ 
                            $sth->execute();
                            return [true, 'Ressource modifiée', 201];
                        }catch(PDOException $e){
                            return [false, "Erreur dans l'execution de la requête", 400];
                        }
        ...
        public function delete($id){ //delete resource by id
            ...
            if(!$result){ //resource doesn't exist
                ...
                if($result[0]["deleted"] == 0){ //exist and wasn't deleted
                    ...
                    try{
                        $sth->execute(); 
                        return [true, 'Resource supprimée', 200];
                    }catch(PDOException $e){ //some error in sql execution
                        return [false, "Erreur dans l'execution de la requête", 400];
                    } 
                }else{ //exist but was deleted
